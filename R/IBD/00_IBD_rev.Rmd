...
centroid <- dplyr::rename(centroid, "ID" = `Sample Name`) #%>% 
  #dplyr::select(-c("metadata_latitude","metadata_longitude","Shehia","Notes"))
...
plot_PCA <- function(df, axis1,axis2,colors_for_plt,symbols_for_plt) {
  plt <- ggplot(data = df, aes(x = {{axis1}}, y = {{axis2}})) +
  # points
  geom_point(aes(color = Sample_Set,shape=Sample_Set), size = 3, show.legend = TRUE) +
  # colouring
...
PCA1and2 <- plot_PCA(pca_df,PC1,PC2,cols,symbols) + 
  xlab(paste0("PC1 (", signif(pca$var[1], 3), "%)")) + 
  ylab(paste0("PC2 (", signif(pca$var[2], 3), "%)"))
  
PCA1and3 <- plot_PCA(pca_df,PC1,PC3,cols,symbols) + 
  xlab(paste0("PC1 (", signif(pca$var[1], 3), "%)")) + 
...
PCA1and2_nohigh <- plot_PCA(pca_df,PC1,PC2,cols,symbols) + 
  xlab(paste0("PC1 (", signif(pca$var[1], 3), "%)")) + 
  ylab(paste0("PC2 (", signif(pca$var[2], 3), "%)"))
PCA1and3_nohigh <- plot_PCA(pca_df,PC1,PC3,cols,symbols) +
  xlab(paste0("PC1 (", signif(pca$var[1], 3), "%)")) + 
  ylab(paste0("PC3 (", signif(pca$var[3], 3), "%)"))
```
...
ggplot() +
  ...
  theme_void() +
  annotation_scale(location = "br",width_hint = 0.25, pad_x = unit(0.0, "in")) +
  #annotate("text", label = "Unguja", x = 40, y = -6)
  geom_text(fontface = "bold",mapping=aes(x=39.66, y=-6.55, label="Unguja"), nudge_y = 0.5, color="black", size = 7) +
  geom_text(fontface = "bold",mapping=aes(x=39.4, y=-5.66, label="Pemba"), nudge_y = 0.5, color="black", size = 7) +
  geom_point(mapping=aes(x=39.2, y=-6.2),show.legend = F,
             color = "red", shape = 15, size = 3, alpha = 0.6)
...
geographic_names <- c("Pemba","Unguja_N", "Unguja_C", "Unguja_S","Mainland")

max_lats <- shehias_to_cluster %>%
  group_by(Cluster) %>% 
  summarize(max_lat = max(lat_centroid)) %>% 
  ungroup() %>% 
  arrange(desc(max_lat)) %>%
  mutate(Geo_Name = geographic_names)

shehias_to_cluster <- shehias_to_cluster %>%
  left_join(max_lats, by = "Cluster")

ordered_geo_names <- shehias_to_cluster %>%
  arrange(desc(max_lat)) %>%
  distinct(Geo_Name, .keep_all = TRUE) %>%
  pull(Geo_Name)

shehias_to_cluster <- shehias_to_cluster %>%
  mutate(Geo_Name = factor(Geo_Name, levels = ordered_geo_names))

col <- brewer.pal(n = length(unique(shehias_to_cluster$Geo_Name)), name = "Set2")
# map of clusters
clust_map <- ggplot() +
  geom_sf(data = tanzania_map) +
  coord_sf(xlim = c(38.7, 40), ylim = c(-7,-4.8), expand = FALSE) +
  geom_point(data = shehias_to_cluster, aes(x = lon_centroid, y = lat_centroid, color = Geo_Name),
             show.legend = T,
  ...
  annotation_scale(location = "br",width_hint = 0.25, pad_x = unit(0.0, "in"))
clust_map2 <- clust_map + theme(legend.position = "none")
le1 <- cowplot::get_legend(clust_map)
clust_map_final <- cowplot::plot_grid(clust_map2, le1, nrow = 2, rel_heights = c(1, 0.2))
ggsave(clust_map_final,file = "results/clust_map.pdf", device = pdf, width = 7, height = 5, units = 'in')

shehias_to_cluster <- shehias_to_cluster %>%
  select(c("Shehia_match","Geo_Name")) %>% 
...
ibd_mle_long_kmeans <-  
  ...
  dplyr::left_join(ibd_mle_long_kmeans,
                   ., 
                   by = "Shehia_match_2") %>% 
  rename(Geo_Name_2 = Geo_Name)

make_subet_df <- function(region1,region2,label) {
  ibd_mle_long_kmeans %>%
  filter(
    (str_detect(Geo_Name_1, region1) & str_detect(Geo_Name_2, region2)) | 
      (str_detect(Geo_Name_1, region2) & str_detect(Geo_Name_2, region1))
    ) %>% 
...
combinations <- as.data.frame(combinations(n = 5,r = 2,v=1:5, repeats.allowed = T))

combinations_named <- sapply(combinations, function(pair) {
  sapply(pair, function(x) geographic_names[x])
})

# pairwise_df <- make_subet_df(as.character(combinations[i,1]),
#                                  as.character(combinations[i,2]),
#                                  paste(as.character(combinations[i,1]),
#                                       as.character(combinations[i,2]),
#                                sep = "-"))

# create pairwise comparison dataframes
for (i in 1:nrow(combinations_named)) {
  pairwise_df <- make_subet_df(as.character(combinations_named[i,1]),
                                 as.character(combinations_named[i,2]),
                                 paste(as.character(combinations_named[i,1]),
                                      as.character(combinations_named[i,2]),
                               sep = "-"))
  plist[[i]] <- pairwise_df %>%
    filter(malecotf > 0 ) %>% 
    ggplot() +
    geom_beeswarm(aes(x = label_to_plt,
                      y = malecotf)) + #,
                 # outlier.shape=4,
                 # outlier.size=10,
                 # alpha = .8) +
...
map <- ggplot() +
  ...
  theme(axis.text.x=element_blank(),
        ...
        axis.title.x=element_blank(),
        axis.title.y=element_blank()) +
  annotation_scale(location = "br",width_hint = 0.25, pad_x = unit(0, "in"))
...
DAPC_gg <- ggplot(data = ind_coords, aes(x = Axis1, y = Axis2)) +
  ...
  # centroids
  geom_label(data = centroid_DAPC, aes(label = Site, color = Site), size = 6, show.legend = FALSE) +
  # colouring
...
DAPC_nohigh <- cowplot::ggdraw() +
  cowplot::draw_plot(DAPC_gg, x = 0, y = 0, width = 1, height = 1, scale = 1) +
  cowplot::draw_plot(map, x = 0.682, y= 0.57, width = 0.4, height = 0.44)

ggsave("results/DAPC_gg_nohigh.pdf", width = 10.3, height = 8)

legend <- get_legend(PCA1and2_nohigh + theme(legend.box.margin = margin(0, 0, 0, 12)))
Figure1A_PC <- plot_grid(NULL, 
                      PCA1and2_nohigh + theme(legend.position="none"), 
                      NULL,
                      PCA1and3_nohigh + theme(legend.position="none"),
...
Figure1A_PC_leg <- plot_grid(Figure1A_PC,
                             legend,
                             ncol = 2,
                             labels = c("",""),
                             rel_widths = c(1, .4))

Figure_1 <- plot_grid(NULL,
                      Figure1A_PC_leg,
                      NULL,
                      DAPC_nohigh,
                      labels = c("","A","","B"),
                      rel_widths = c(0, 1, -0.15, 1),
                      label_size = 18,
                      nrow = 1)
ggsave("results/Figure1.pdf", Figure_1,width = 21, height = 9)
ggsave("results/Figure1.png", Figure_1,width = 21, height = 9)
```
...
map <- ggplot() +
  ...
  theme(axis.text.x=element_blank(),
        ...
        axis.title.x=element_blank(),
        axis.title.y=element_blank()) +
  annotation_scale(location = "br",width_hint = 0.25, pad_y = unit(0, "in"))
...
DAPC <- cowplot::ggdraw() +
  cowplot::draw_plot(DAPC_gg, x = 0, y = 0, width = 1, height = 1, scale = 1) +
  cowplot::draw_plot(map, x = 0.52, y= 0.47, width = 0.6, height = 0.55)

ggsave("results/DAPC_gg.pdf", DAPC, width = 8, height = 6, units = 'in')

legend <- get_legend(PCA1and2 + theme(legend.box.margin = margin(0, 0, 0, 12)))
FigureS4A_PC <- plot_grid(NULL, 
                      PCA1and2 + theme(legend.position="none"), 
                      NULL,
                      PCA1and3 + theme(legend.position="none"),
                      # A negative rel_height shrinks space between elements
                      rel_heights = c(0.1, 1, 0, 1),
                      ncol = 1,
                      label_y = 1.07,
                      labels = c("","","",""),
                      label_fontfamily = "sans")
FigureS4A_PC_leg <- plot_grid(FigureS4A_PC,
                             legend,
                             ncol = 2,
                             labels = c("",""),
                             rel_widths = c(1, .4))

Figure_S4 <- plot_grid(NULL,
                      FigureS4A_PC_leg,
                      NULL,
                      DAPC,
                      labels = c("","A","","B"),
                      ...
                      label_size = 18,
                      nrow = 1)
ggsave("results/FigureS4.pdf", Figure_S4,width = 14, height = 8)
ggsave("results/FigureS4.png", Figure_S4,width = 14, height = 8)
```
...
Unguja_plot <- ggplot() +
  ...
  geom_point(data = withinpairs, aes(x = lon_centroid_1, y = lat_centroid_1, colour = malecotf),
             ...
             shape = 15, 
             size = 6) +
  #scale_colour_continuous_sequential(palette = "Greens") +
  scale_colour_gradient(low='grey80',high='dark green') + 
  # add label that blue = high within pair freq
  # ggrepel::geom_label_repel(data = Unguja_coordinates_toplot, aes(lon_centroid, lat_centroid, label = Shehia_match), colour = "black", label.size = 0.05,nudge_x = -.02,nudge_y = 0.02) +
  theme_void() +
  labs(colour = "Within IBD") +
  ...
  annotation_scale(location = "br",width_hint = 0.25, pad_x = unit(0.0, "in"))
Unguja_plot
ggsave("results/Unguja_0.25_revised.pdf", plot = Unguja_plot,device = pdf, width = 10, height = 10, units = "in", dpi = 300, bg = 'white')
...
Pemba_plot <- ggplot() +
  ...
  theme(legend.text = element_text(size = 30),
        legend.title = element_text(size = 45))
Pemba_plot
ggsave("results/Pemba_0.25_revised.pdf", plot = Pemba_plot,device = pdf, width = 10, height = 10, units = "in", dpi = 300, bg = 'white')
...
Pemba_net <- shehia_analysis_Pemba %>%
  ...
  scale_color_manual(values = getPalette,
                     name = "Shehia") +
  ggraph::scale_edge_colour_viridis("IBD", 
                                    begin = 0.25, 
                                    end = 1, 
                                    option = "plasma",
                                    guide = guide_colorbar(available_aes = "edge_colour")) +
  theme_graph() +
  theme(legend.position = "bottom",text = element_text(size = 14,family = "sans")) 
...
Pemba_shehia_color <- ggplot() +
  ...
  geom_point(data = Shehias_to_plot, aes(x = lon_centroid, y = lat_centroid, color = Shehia_match),
             show.legend = F,
             color = getPalette, shape = 15, size = 12) +
  # ggrepel::geom_label_repel(data = Shehias_to_plot, 
  #                           aes(lon_centroid, lat_centroid, label = Shehia_match, colour = Shehia_match), 
  #                           colour = getPalette, 
  #                           size = 8, 
  #                           label.size = NA,
  #                           nudge_x = -0.001, 
...
ggsave(combined, file = "results/map_net_Unguja.pdf", device = pdf, width = 35, height = 49, units = "in")
ggsave(combined, file = "results/map_net_Unguja.png", device = png, width = 35, height = 49, units = "in")
...
ggsave("results/connectivity_0.25_or_greater_mainland.png", device = png, width = 40, height = 40, units = "in")
```
...
common_theme <- theme_classic() +
  theme(axis.text = element_text(size = 55, family = "sans"),
        axis.text.y = element_text(size = 55, family = "sans"),
        axis.title.y = element_text(size = 55, family = "sans"),
        axis.title = element_text(size = 65, family = "sans"))

# Apply this theme to all individual plots
main_Zanzibar <- main_Zanzibar + common_theme
main_Unguja <- main_Unguja + common_theme
main_Pemba <- main_Pemba + common_theme
inset_Zanzibar <- inset_Zanzibar + common_theme
inset_Unguja <- inset_Unguja + common_theme

combined_plot_all <- plot_grid(
  inset_Zanzibar,
  inset_Unguja,
  main_Pemba,
  nrow = 3,
...
combined_plot_sup <- plot_grid(
  ...
  label_size = 50,
  align = 'v'
) + xlab("Spatial Separation")

# Save the combined plots
ggsave("results/isolation_by_distance_all.pdf", combined_plot_all, device = "pdf", width = 35, height = 35, units = 'in')
ggsave("results/isolation_by_distance_sup.pdf", combined_plot_sup, device = "pdf", width = 35, height = 35, units = 'in')
ggsave("results/isolation_by_distance_all.png", combined_plot_all, device = "png", width = 35, height = 35, units = 'in')
ggsave("results/isolation_by_distance_sup.png", combined_plot_sup, device = "png", width = 35, height = 35, units = 'in')
```
